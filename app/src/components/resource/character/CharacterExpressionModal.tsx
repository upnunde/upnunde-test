...
export function CharacterExpressionModal({
  ...
  cropAspect = "9/16",
  cropGuide: cropGuideProp,
  showCropGuide: showCropGuideProp,
}: CharacterExpressionModalProps) {
  const cropGuideEnabled = cropGuideProp ?? showCropGuideProp ?? showExpressionSection;
  ...
  /** 9:16 가이드(좌우 딤): 정사각 스테이지 높이 = cropStagePx, 가로 = round(세로×9/16) */
  const cropStagePx = layoutShowSlotList ? 320 : 400;
  const posterViewportW = Math.round((cropStagePx * 9) / 16);
  const posterSidePadPx = (cropStagePx - posterViewportW) / 2;
  ...
  const handleSave = useCallback(async () => {
    const { viewportW, viewportH } = getCropViewportDimensions(layoutShowSlotList, cropAspect);
    ...
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      ...
      >
        ...
        >
          ...
          >
            {/* 크롭 영역: 9:16+가이드는 썸네일과 동일한 정사각 스테이지 + 좌우 딤 */}
            {cropAspect === "9/16" && cropGuideEnabled ? (
              <div
                className={`relative flex shrink-0 flex-row items-stretch overflow-hidden rounded-md bg-[repeating-conic-gradient(#e2e8f0_0%_25%,#f1f5f9_0%_50%)] bg-[length:12px_12px] ${
                  layoutShowSlotList ? "h-[320px] w-[320px]" : "h-[400px] w-[400px]"
                }`}
              ...
              >
                <div className="shrink-0 bg-black/70" style={{ width: posterSidePadPx }} aria-hidden />
                <div
                  className="relative h-full shrink-0 overflow-hidden"
                  style={{ width: posterViewportW }}
                >
                  {previewImageUrl ? (
                    <>
                      <div
                      ...
                      <div
                        className="pointer-events-none absolute inset-0 outline outline-1 outline-offset-[-1px] outline-slate-200/85"
                        aria-hidden
                      />
                      <div className="pointer-events-none absolute inset-0" aria-hidden>
                        <div className="absolute left-0 top-0 h-3 w-3 border-l border-t border-slate-200/85" />
                        <div className="absolute right-0 top-0 h-3 w-3 border-r border-t border-slate-200/85" />
                        <div className="absolute bottom-0 left-0 h-3 w-3 border-b border-l border-slate-200/85" />
                        <div className="absolute bottom-0 right-0 h-3 w-3 border-b border-r border-slate-200/85" />
                      </div>
                    </>
                  ) : (
                    <span className="absolute inset-0 flex items-center justify-center px-1 text-center text-on-surface-30 text-sm">
                      썸네일을 클릭하여 선택하세요
                    </span>
                  )}
                </div>
              ...
              <div
                className={`${
                  cropAspect === "square"
                    ? layoutShowSlotList
                      ? "h-[320px] w-[320px] aspect-square"
                      : "h-[400px] w-[400px] aspect-square"
                    : layoutShowSlotList
                      ? "h-[320px] aspect-[9/16] w-auto max-w-full"
                      : "h-[400px] aspect-[9/16] w-auto"
                } relative rounded-md overflow-hidden bg-[repeating-conic-gradient(#e2e8f0_0%_25%,#f1f5f9_0%_50%)] bg-[length:12px_12px] flex items-center justify-center`}
                onPointerDown={handleCropPointerDown}
                onPointerMove={handleCropPointerMove}
                onPointerUp={handleCropPointerUp}
                onPointerCancel={handleCropPointerUp}
